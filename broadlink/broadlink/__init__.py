...
def gendevice(devtype, host, mac, timeout = 10):
  if devtype == 0: # SP1
    return sp1(devtype=devtype, host=host, mac=mac, type="SP1", timeout=timeout)
  if devtype == 0x2711: # SP2
    return sp2(devtype=devtype, host=host, mac=mac, type="SP2", timeout=timeout)
  if devtype == 0x2719 or devtype == 0x7919 or devtype == 0x271a or devtype == 0x791a: # Honeywell SP2
    return sp2(devtype=devtype, host=host, mac=mac, type="Honeywell SP2", timeout=timeout)
  if devtype == 0x2720: # SPMini
    return sp2(devtype=devtype, host=host, mac=mac, type="SPMini", timeout=timeout)
  elif devtype == 0x753e: # SP3
    return sp2(devtype=devtype, host=host, mac=mac, type="SP3", timeout=timeout)
  elif devtype == 0x2728: # SPMini2
    return sp2(devtype=devtype, host=host, mac=mac, type="SPMini2", timeout=timeout)
  elif devtype == 0x2733 or devtype == 0x273e: # OEM branded SPMini
    return sp2(devtype=devtype, host=host, mac=mac, type="OEM branded SPMini", timeout=timeout)
  elif devtype >= 0x7530 and devtype <= 0x7918: # OEM branded SPMini2
    return sp2(devtype=devtype, host=host, mac=mac, type="OEM branded SPMini2", timeout=timeout)
  elif devtype == 0x2736: # SPMiniPlus
    return sp2(devtype=devtype, host=host, mac=mac, type="SPMiniPlus", timeout=timeout)
  elif devtype == 0x2712: # RM2
    return rm(devtype=devtype, host=host, mac=mac, type="RM2", timeout=timeout)
  elif devtype == 0x2737: # RM Mini
    return rm(devtype=devtype, host=host, mac=mac, type="RM Mini", timeout=timeout)
  elif devtype == 0x273d: # RM Pro Phicomm
    return rm(devtype=devtype, host=host, mac=mac, type="RM Pro Phicom", timeout=timeout)
  elif devtype == 0x2783: # RM2 Home Plus
    return rm(devtype=devtype, host=host, mac=mac, type="RM2 Home Plus", timeout=timeout)
  elif devtype == 0x277c: # RM2 Home Plus GDT
    return rm(devtype=devtype, host=host, mac=mac, type="RM2 Home Plus GDT", timeout=timeout)
  elif devtype == 0x272a: # RM2 Pro Plus
    return rm(devtype=devtype, host=host, mac=mac, type="RM2 Pro Plus", timeout=timeout)
  elif devtype == 0x2787: # RM2 Pro Plus2
    return rm(devtype=devtype, host=host, mac=mac, type="RM2 Pro Plus2", timeout=timeout)
  elif devtype == 0x278b: # RM2 Pro Plus BL
    return rm(devtype=devtype, host=host, mac=mac, type="RM2 Pro Plus BL", timeout=timeout)
  elif devtype == 0x278f: # RM Mini Shate
    return rm(devtype=devtype, host=host, mac=mac, type="RM Mini Shate", timeout=timeout)
  elif devtype == 0x2714: # A1
    return a1(devtype=devtype, host=host, mac=mac, type="A1", timeout=timeout)
  elif devtype == 0x4EB5: # MP1
    return mp1(devtype=devtype, host=host, mac=mac, type="MP1", timeout=timeout)
  else:
    return device(devtype=devtype, host=host, mac=mac, timeout=timeout)
...
class device:
  def __init__(self, devtype, host, mac, type="Unknown", timeout=10):
    self.host = host
    self.mac = mac
...
class mp1(device):
  def __init__ (self, devtype, host, mac, type, timeout):
    device.__init__(self, devtype, host, mac, type, timeout)
    self.type = type
    self.devtype = devtype
